...
class ScriptMaker(object):
    ...
    def _get_shebang(self, encoding, post_interp=b'', options=None):
        ...
        if self.executable:
            ...
            executable = os.path.join(sysconfig.get_path('scripts'), f"python{sysconfig.get_config_var('EXE')}")

        else:  # pragma: no cover
            executable = os.path.join(sysconfig.get_config_var('BINDIR'), f"python{sysconfig.get_config_var('VERSION')}{sysconfig.get_config_var('EXE')}")

            if not os.path.isfile(executable):
                # for Python builds from source on Windows, no Python executables with
                # a version suffix are created, so we use python.exe
                executable = os.path.join(sysconfig.get_config_var('BINDIR'), f"python{sysconfig.get_config_var('EXE')}")
        ...
        try:
            ...
            raise ValueError('The shebang (%r) is not decodable from utf-8' % shebang) from e
        ...
        if encoding != 'utf-8':
            try:
                ...
                raise ValueError('The shebang (%r) is not decodable ' 'from the script encoding (%r)' % (shebang, encoding)) from exc
