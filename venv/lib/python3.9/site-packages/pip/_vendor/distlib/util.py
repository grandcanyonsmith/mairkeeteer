...
def parse_requirement(req):
    ...
    rs = f"{distname} {', '.join(['%s %s' % con for con in versions])}" if versions else distname
...
def in_venv():
    return True if hasattr(sys, 'real_prefix') else sys.prefix != getattr(sys, 'base_prefix', sys.prefix)
...
class FileOperator(object):
    ...
    def newer(self, source, target):
        ...
        return os.stat(source).st_mtime > os.stat(target).st_mtime if os.path.exists(target) else True
...
class ExportEntry(object):
    ...
    def __eq__(self, other):
        return (self.name == other.name and self.prefix == other.prefix and self.suffix == other.suffix and self.flags == other.flags) if isinstance(other, ExportEntry) else False
...
class Sequencer(object):
    ...
    def remove(self, pred, succ):
        ...
        try:
            ...
            raise ValueError('%r not a successor of anything' % succ) from e
        try:
            preds.remove(pred)
            succs.remove(succ)
        except KeyError as exc:  # pragma: no cover
            raise ValueError('%r not a successor of %r' % (succ, pred)) from exc
...
class Progress(object):
    ...
    def format_duration(self, duration):
        return '??:??:??' if (duration <= 0) and self.max is None or self.cur == self.min else time.strftime('%H:%M:%S', time.gmtime(duration))
        ...
        def https_open(self, req):
            try:
                ...
                if 'certificate verify failed' in str(e.reason):
                    raise CertificateError('Unable to verify server certificate ' 'for %s' % req.host) from e
    ...
    def read(self):
        ...
        if os.path.exists(self.filename):
            ...
            if 'distutils' in sections:
                ...
                if _servers := [server.strip() for server in index_servers.split('\n') if server.strip() != '']:
                    for server in _servers:
                        ...
                        for key, default in (('repository', self.DEFAULT_REPOSITORY), ('realm', self.DEFAULT_REALM), ('password', None)):
                            result[key] = config.get(server, key) if config.has_option(server, key) else default
