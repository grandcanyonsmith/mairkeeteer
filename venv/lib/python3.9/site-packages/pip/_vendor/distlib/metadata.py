...
def _best_version(fields):
    ...
    for key in keys:
        ...
        if key not in _566_FIELDS and '2.1' in possible_versions and key != 'Description':
    ...
    if not is_1_1 and not is_1_2 and not is_2_1 and not is_2_0 and PKG_INFO_PREFERRED_VERSION in possible_versions:
...
class Metadata(object):
            ...
            if self._legacy:
                ...
                if key not in ('commands', 'exports', 'modules', 'namespaces',
                    ...
                    if d := self._data.get('extensions'):
                        ...
                        elif d := d.get('python.exports') or self._data.get('python.exports'):
                            result = d.get(key, value)
    ...
    def _validate_mapping(self, mapping, scheme):
        ...
        if missing := [key for key, exclusions in self.MANDATORY_KEYS.items() if key not in mapping and scheme not in exclusions]:
    ...
    def add_requirements(self, requirements):
        if self._legacy:
            ...
            always = next((entry for entry in run_requires if 'environment' not in entry and 'extra' not in entry), None)
