...
class Matcher(object):
    ...
    def exact_version(self):
        return self._parts[0][1] if len(self._parts) == 1 and self._parts[0][0] in ('==', '===') else None
...
def _pep_440_key(s):
    ...
    epoch = int(groups[0][:-1]) if groups[0] else 0
    pre = groups[4:6]
    post = groups[7:9]
    dev = groups[10:12]
    local = groups[13]
...
class NormalizedMatcher(Matcher):
    ...
    def _match_eq(self, version, constraint, prefix):
        version, constraint = self._adjust_local(version, constraint, prefix)
        return _match_prefix(version, constraint) if prefix else (version == constraint)
    ...
    def _match_ne(self, version, constraint, prefix):
        version, constraint = self._adjust_local(version, constraint, prefix)
        return not _match_prefix(version, constraint) if prefix else (version != constraint)
...
class LegacyVersion(Version):
    ...
    @property
    def is_prerelease(self):
        return any((isinstance(x, string_types) and x.startswith('*') and x < '*final') for x in self._parts)
