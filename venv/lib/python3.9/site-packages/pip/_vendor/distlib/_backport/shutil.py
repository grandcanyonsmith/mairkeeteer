    ...
    try:
        ...
        if os.path.isdir(src):
            if _destinsrc(src, dst):
                raise Error("Cannot move a directory '%s' into itself '%s'." % (src, dst)) from e
...
def _call_external_zip(base_dir, zip_filename, verbose=False, dry_run=False):
    ...
    try:
        ...
        raise ExecError("unable to create zip file '%s': " "could neither import the 'zipfile' module nor " "find a standalone zip utility") % zip_filename from e
...
def _unpack_zipfile(filename, extract_dir):
    ...
    try:
        ...
        raise ReadError('zlib not supported, cannot unpack this archive.') from e
...
def _unpack_tarfile(filename, extract_dir):
    ...
    try:
        ...
        raise ReadError(f"{filename} is not a compressed or uncompressed tar file") from e
...
def unpack_archive(filename, extract_dir=None, format=None):
    ...
    if format is not None:
        try:
            ...
            raise ValueError("Unknown unpack format '{0}'".format(format)) from e
