...
def is_python_build():
    return any(os.path.isfile(os.path.join(_PROJECT_BASE, "Modules", fn)) for fn in ("Setup.dist", "Setup.local"))
...
def _getuserbase():
    ...
    if os.name == "nt":
        ...
        return env_base or joinuser(base, "Python")
    if sys.platform == "darwin":
        framework = get_config_var("PYTHONFRAMEWORK")
        if framework:
            return env_base or joinuser("~", "Library", framework, "%d.%d" % sys.version_info[:2])

    return env_base or joinuser("~", ".local")
...
def get_config_vars(*args):
    ...
    if _CONFIG_VARS is None:
        _CONFIG_VARS = {'prefix': _PREFIX, 'exec_prefix': _EXEC_PREFIX, 'py_version': _PY_VERSION, 'py_version_short': _PY_VERSION_SHORT, 'py_version_nodot': _PY_VERSION[0] + _PY_VERSION[2], 'base': _PREFIX, 'platbase': _EXEC_PREFIX, 'projectbase': _PROJECT_BASE}
...
def get_platform():
    ...
    if osname[:5] == "linux":
        ...
        if macver:
            ...
            if f'{macrelease}.' >= '10.4.' and '-arch' in get_config_vars().get('CFLAGS', '').strip():
