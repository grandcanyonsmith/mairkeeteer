        ...
        if len(versions) > 2:   # urls and digests keys are present
            ...
            for k in versions:
                if k in ('urls', 'digests'):
                    continue
                try:
                    if matcher.match(k) and (prereleases or not vcls(k).is_prerelease):
                        slist.append(k)
...
class DistPathLocator(Locator):
    ...
    def _get_project(self, name):
        ...
        return {'urls': {}, 'digests': {}} if dist is None else {dist.version: dist, 'urls': {dist.version: {dist.source_url}}, 'digests': {dist.version: {None}}}
