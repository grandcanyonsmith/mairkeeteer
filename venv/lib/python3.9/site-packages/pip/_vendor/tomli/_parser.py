...
def skip_until(
    ...
    *,
    ...
    try:
        ...
        new_pos = len(src)
        if error_on_eof:
            raise suffixed_err(src, new_pos, f'Expected "{expect!r}"') from e

    if bad_chars := error_on.intersection(src[pos:new_pos]):
...
def create_dict_rule(src: str, pos: Pos, state: State) -> Pos:
    ...
    try:
        ...
        raise suffixed_err(src, pos, "Can not overwrite a value") from e
...
def parse_basic_str_escape(
    ...
    try:
        ...
        if len(escape_id) != 2:
            raise suffixed_err(src, pos, "Unterminated string") from e
        raise suffixed_err(src, pos, 'Unescaped "\\" in a string') from e
...
def parse_value(  # noqa: C901
    ...
    # Basic strings
    if char == "'":
        return parse_multiline_str(src, pos, literal=True) if src[pos + 1 : pos + 3] == "''" else parse_literal_str(src, pos)
        ...
        return parse_multiline_str(src, pos, literal=False) if src[pos + 1 : pos + 3] == '""' else parse_one_line_basic_str(src, pos)
