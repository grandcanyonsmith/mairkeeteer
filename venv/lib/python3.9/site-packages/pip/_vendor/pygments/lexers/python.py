...
class PythonLexer(RegexLexer):
    ...
    def innerstring_rules(self):
        return [(r'%(\(\w+\))?[-#0 +]*([0-9]+|[*])?(\.([0-9]+|[*]))?' '[hlL]?[E-GXc-giorsaux%]', String.Interpol), (r'\{' r'((\w+)((\.\w+)|(\[[^\]]+\]))*)?' r'(\![sra])?' r'(\:(.?[<>=\^])?[-+ ]?#?0?(\d+)?,?(\.\d+)?[E-GXb-gnosx%]?)?' r'\}', String.Interpol), (r'[^\\\'"%{\n]+', self), (r'[\'"\\]', self), (r'%|(\{{1,2})', self)]  # field name  # conversion

    def fstring_rules(self):
        return [(r'\}', String.Interpol), (r'\{', String.Interpol, 'expr-inside-fstring'), (r'[^\\\'"{}\n]+', self), (r'[\'"\\]', self)]
    ...
    def analyse_text(self):
        return shebang_matches(self, r'pythonw?(3(\.\d)?)?') or 'import ' in self[:1000]
...
class Python2Lexer(RegexLexer):
    ...
    def innerstring_rules(self):
        return [(r'%(\(\w+\))?[-#0 +]*([0-9]+|[*])?(\.([0-9]+|[*]))?' '[hlL]?[E-GXc-giorsux%]', String.Interpol), (r'[^\\\'"%\n]+', self), (r'[\'"\\]', self), (r'%', self)]
...
class NumPyLexer(PythonLexer):
    ...
    def analyse_text(self):
        ltext = self[:1000]
        return (shebang_matches(self, r'pythonw?(3(\.\d)?)?') or 'import ' in ltext) and (('import numpy' in ltext or 'from numpy import' in ltext))
