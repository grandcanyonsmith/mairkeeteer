...
def get_style_by_name(name):
    if name in STYLE_MAP:
        ...
        builtin = ""
        mod = name
        cls = f"{name.title()}Style"

    try:
        mod = __import__(f'pygments.styles.{mod}', None, None, [cls])
    except ImportError as e:
        raise ClassNotFound("Could not find style module %r" % mod + (builtin and ", though it should be builtin") + ".") from e

    try:
        return getattr(mod, cls)
    except AttributeError as exc:
        raise ClassNotFound("Could not find style class %r in style module." % cls) from exc
