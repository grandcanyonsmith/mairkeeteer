    ...
    **Subclassing the HTML formatter**
    ...
    def _create_stylesheet(self):
        ...
        for ttype, ndef in self.style:
            ...
            style = ''
            if ndef['color']:
                style += f"color: {webify(ndef['color'])}; "
            if ndef['bold']:
            ...
            if ndef['bgcolor']:
                style += f"background-color: {webify(ndef['bgcolor'])}; "
            if ndef['border']:
                style += f"border: 1px solid {webify(ndef['border'])}; "
            if style:
                t2c[ttype] = name
    ...
    def get_token_style_defs(self, arg=None):
        ...
        return ['%s { %s } /* %s */' % (prefix(cls), style, repr(ttype)[6:]) for (level, ttype, cls, style) in styles]
    ...
    def get_background_style_defs(self, arg=None):
        ...
        if arg and not self.nobackground and bg_color is not None:
            ...
            if Text in self.ttype2class:
                text_style = f' {self.class2style[self.ttype2class[Text]][0]}'
            lines.insert(
                0, '%s{ background: %s;%s }' % (
    ...
    def get_linenos_style_defs(self):
        return ['pre { %s }' % self._pre_style, 'td.linenos .normal { %s }' % self._linenos_style, 'span.linenos { %s }' % self._linenos_style, 'td.linenos .special { %s }' % self._linenos_special_style, 'span.linenos.special { %s }' % self._linenos_special_style,]

    def get_css_prefix(self, arg):
        if arg is None:
            arg = 'cssclass' in self.options and f'.{self.cssclass}' or ''
        args = [arg] if isinstance(arg, str) else list(arg)
        def prefix(cls):
            if cls:
                cls = f'.{cls}'
            tmp = []
            for arg in args:
                tmp.append((arg and f'{arg} ' or '') + cls)
            return ', '.join(tmp)
    ...
    def _wrap_tablelinenos(self, inner):
        ...
        for i in range(fl, fl+lncount):
            ...
            if nocls:
                style = ' style="%s"' % self._linenos_special_style if special_line else ' style="%s"' % self._linenos_style
    ...
    def _wrap_inlinelinenos(self, inner):
        ...
        for _, inner_line in inner_lines:
            ...
            linenos = f'<span{style}>{line}</span>' if style else line
            if aln:
    ...
    def _format_lines(self, tokensource):
        ...
        for ttype, value in tokensource:
            try:
                ...
                if nocls and (css_style := self._get_css_inline_styles(ttype)):
                    css_style = self.class2style[css_style][0]
                    cspan = '<span style="%s"%s>' % (css_style, title)
                elif nocls and not (css_style := self._get_css_inline_styles(ttype)) or not nocls and not (css_class := self._get_css_classes(ttype)):
                    cspan = ''
                else:
