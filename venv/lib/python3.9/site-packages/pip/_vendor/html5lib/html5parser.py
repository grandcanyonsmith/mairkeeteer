...
class HTMLParser(object):
    ...
    def documentEncoding(self):
        ...
        """
        return self.tokenizer.stream.charEncoding[0].name if hasattr(self, 'tokenizer') else None
...
def getPhases(debug):
    def log(function):
        ...
        def wrapped(self, *args, **kwargs):
            if function.__name__.startswith("process") and args:
                token = args[0]
    ...
    class TextPhase(Phase):
        ...
        def startTagOther(self, token):
            assert False, f"Tried to process start tag {token['name']} in RCDATA/RAWTEXT mode"
...
def adjust_attributes(token, replacements):
    if needs_adjustment := viewkeys(token['data']) & viewkeys(replacements):
