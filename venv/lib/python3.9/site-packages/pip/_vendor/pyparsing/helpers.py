...
def delimited_list(
    ...
    *,
    ...
    dlName = "{expr} [{delim} {expr}]...{end}".format(expr=str(expr.copy().streamline()), delim=str(delim), end=f" [{str(delim)}]" if allow_trailing_delim else "")
...
def nested_expr(
    ...
    *,
    ...
    if content is None:
        if not isinstance(opener, str_type) or not isinstance(closer, str_type):
            raise ValueError(
                "opening and closing arguments must be strings if no content expression is given"
            )
        if len(opener) == 1 and len(closer) == 1:
            content = Combine(OneOrMore(~ignoreExpr + CharsNotIn(opener + closer + ParserElement.DEFAULT_WHITE_CHARS, exact=1,))).set_parse_action(lambda t: t[0].strip()) if ignoreExpr is not None else empty.copy() + CharsNotIn(opener + closer + ParserElement.DEFAULT_WHITE_CHARS).set_parse_action(lambda t: t[0].strip())
...
any_open_tag, any_close_tag = make_html_tags(Word(alphas, f"{alphanums}_:").set_name("any tag"))
    ...
    for operDef in op_list:
        ...
        if rightLeftAssoc is OpAssoc.LEFT:
            if arity == 1:
                ...
                matchExpr = _FB(lastExpr + opExpr + lastExpr) + Group(lastExpr + (opExpr + lastExpr)[1, ...]) if opExpr is not None else _FB(lastExpr + lastExpr) + Group(lastExpr[2, ...])
