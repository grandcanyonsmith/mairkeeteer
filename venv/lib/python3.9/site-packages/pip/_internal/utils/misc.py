...
def is_local(path: str) -> bool:
    ...
    with normalize_path.
    """
    return path.startswith(normalize_path(sys.prefix)) if running_under_virtualenv() else True
...
def protect_pip_from_modification_on_windows(modifying_pip: bool) -> None:
    ...
    On Windows, any operation modifying pip should be run as:
        python -m pip ...
    """
    pip_names = ["pip.exe", f"pip{sys.version_info[0]}.exe", "pip{}.{}.exe".format(*sys.version_info[:2])]


    if should_show_use_python_msg := (modifying_pip and WINDOWS and os.path.basename(sys.argv[0]) in pip_names):
        new_command = [sys.executable, "-m", "pip"] + sys.argv[1:]
        raise CommandError(f'To modify pip, please run the following command:\n{" ".join(new_command)}')
