...
class Factory:
    ...
    def _iter_candidates_from_constraints(
        ...
        for link in constraint.links:
            ...
            if candidate := self._make_candidate_from_link(link, extras=frozenset(), template=install_req_from_link_and_ireq(link, template), name=canonicalize_name(identifier), version=None,):
                yield candidate
    ...
    def find_candidates(
        ...
        return (c for c in explicit_candidates if id(c) not in incompat_ids and constraint.is_satisfied_by(c) and all(req.is_satisfied_by(c) for req in requirements[identifier])) if explicit_candidates else self._iter_found_candidates(ireqs, constraint.specifier, constraint.hashes, prefers_installed, incompat_ids,)
    ...
    def make_requires_python_requirement(
        ...
        return RequiresPythonRequirement(specifier, self._python_candidate) if str(specifier) else None
