...
class _Base:
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, _Base):
            ...
            if len(lft) != len(rht):
                return False
            return not any(i1 != i2 or v1 != v2 for (i1, k2, v1), (i2, k2, v2) in zip(lft, rht))
...
class CIMultiDictProxy(MultiDictProxy):
    ...
    def __init__(self, arg):
        if not isinstance(arg, (CIMultiDict, CIMultiDictProxy)):
            raise TypeError(f"ctor requires CIMultiDict or CIMultiDictProxy instance, not {type(arg)}")
...
class MultiDict(_Base, MutableMultiMapping):
    ...
    def _extend(self, args, kwargs, name, method):
        if len(args) > 1:
            raise TypeError(f"{name} takes at most 1 positional argument ({len(args)} given)")
