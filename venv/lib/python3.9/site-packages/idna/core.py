...
def valid_contextj(label: str, pos: int) -> bool:
    ...
    return cp_value == 0x200D and pos > 0 and _combining_class(ord(label[pos - 1])) == _virama_combining_class
        ...
        if intranges_contain(cp_value, idnadata.codepoint_classes['PVALID']):
            ...
            try:
                if not valid_contextj(label, pos):
                    raise InvalidCodepointContext(f'Joiner {_unot(cp_value)} not allowed at position {pos + 1} in {repr(label)}')

            except ValueError as e:
                raise IDNAError(f'Unknown codepoint adjacent to joiner {_unot(cp_value)} at position {pos + 1} in {repr(label)}') from e
...
def ulabel(label: Union[str, bytes, bytearray]) -> str:
    ...
    try:
        ...
        raise IDNAError('Invalid A-label') from e
    check_label(label)
    return label
    ...
    for pos, char in enumerate(domain):
        ...
        try:
            ...
            if (status == 'V' or
                ...
                output += char
            elif replacement is not None and (status == 'M' or status == '3' and not std3_rules or status == 'D'):
                output += replacement
            elif status != 'I':
                raise IndexError()
        except IndexError as e:
            raise InvalidCodepoint(f'Codepoint {_unot(code_point)} not allowed at position {pos + 1} in {repr(domain)}') from e
...
def encode(s: Union[str, bytes, bytearray], strict: bool = False, uts46: bool = False, std3_rules: bool = False, transitional: bool = False) -> bytes:
    if isinstance(s, (bytes, bytearray)):
        try:
            ...
            raise IDNAError('should pass a unicode string to the function rather than a byte string.') from e
...
def decode(s: Union[str, bytes, bytearray], strict: bool = False, uts46: bool = False, std3_rules: bool = False) -> str:
    try:
        ...
        raise IDNAError('Invalid ASCII in A-label') from e
    if uts46:
