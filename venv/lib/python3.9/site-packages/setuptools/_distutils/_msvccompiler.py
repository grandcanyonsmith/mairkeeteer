...
def _get_vc_env(plat_spec):
    ...
    try:
        out = subprocess.check_output(f'cmd /u /c "{vcvarsall}" {plat_spec} && set', stderr=subprocess.STDOUT).decode('utf-16le', errors='replace')

    except subprocess.CalledProcessError as exc:
        log.error(exc.output)
        raise DistutilsPlatformError(f"Error executing {exc.cmd}") from exc

    return {key.lower(): value for key, _, value in (line.partition('=') for line in out.splitlines()) if key and value}
...
class MSVCCompiler(CCompiler) :
    ...
    def link(self,
        ...
        if self._need_link(objects, output_filename):
            ...
            ld_args = ldflags + lib_opts + export_opts + objects + [f'/OUT:{output_filename}']
