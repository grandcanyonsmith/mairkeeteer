...
def _is_python_source_dir(d):
    return any(os.path.isfile(os.path.join(d, "Modules", fn)) for fn in ("Setup", "Setup.local"))
...
def get_python_inc(plat_specific=0, prefix=None):
    ...
    if os.name == "nt":
        return (os.path.join(prefix, "include") + os.path.pathsep + os.path.join(prefix, "PC")) if python_build else os.path.join(prefix, "include")
...
def customize_compiler(compiler):
    ...
    cc_cmd = f'{cc} {cflags}'
    compiler.set_executables(preprocessor=cpp, compiler=cc_cmd, compiler_so=f'{cc_cmd} {ccshared}', compiler_cxx=cxx, linker_so=ldshared, linker_exe=cc, archiver=archiver)
