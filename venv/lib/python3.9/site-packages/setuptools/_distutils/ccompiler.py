...
class CCompiler:
    ...
    def _need_link(self, objects, output_file):
        ...
        if self.force:
            ...
            return newer_group(objects, output_file, missing='newer') if self.dry_run else newer_group(objects, output_file)
...
def get_default_compiler(osname=None, platform=None):
    ...
    return next((compiler for pattern, compiler in _default_compilers if re.match(pattern, platform) is not None or re.match(pattern, osname) is not None), 'unix')
...
def show_compilers():
    ...
    compilers = [(f"compiler={compiler}", None, compiler_class[compiler][2]) for compiler in compiler_class.keys()]
...
def new_compiler(plat=None, compiler=None, verbose=0, dry_run=0, force=0):
    ...
    try:
        ...
        raise DistutilsModuleError("can't compile C/C++ code: unable to load module '%s'" % module_name) from exc

    except KeyError as exc:
        raise DistutilsModuleError("can't compile C/C++ code: unable to find class '%s' " "in module '%s'" % (class_name, module_name)) from exc
