...
def get_macosx_target_ver():
    ...
    if env_ver := os.environ.get(MACOSX_VERSION_VAR):
        ...
        if syscfg_ver and split_version(syscfg_ver) >= [10, 3] and \
            ...
            my_msg = f'${MACOSX_VERSION_VAR}' + ' mismatch: ' 'now "%s" but "%s" during configure; ' 'must use 10.3 or later' % (env_ver, syscfg_ver)
...
def change_root(new_root, pathname):
    ...
    if os.name == 'nt':
        ...
        return os.path.join(new_root, pathname[1:]) if os.path.isabs(pathname) else os.path.join(new_root, pathname)
