...
class HTTPResponse(io.IOBase):
    ...
    def _fp_read(self, amt):
        ...
        assert self._fp
        c_int_max = 2 ** 31 - 1
        if (not amt or amt <= c_int_max) and (not self.length_remaining or self.length_remaining <= c_int_max) or util.IS_SECURETRANSPORT or not util.IS_PYOPENSSL and sys.version_info >= (3, 10):
    ...
    def from_httplib(cls, r, **response_kw):
        ...
        return cls(body=r, headers=headers, status=r.status, version=r.version, reason=r.reason, strict=strict, original_response=r, **response_kw)
    ...
    def _update_chunk_length(self):
        ...
        try:
            ...
            self.close()
            raise InvalidChunkLength(self, line) from e
    ...
    def read_chunked(self, amt=None, decode_content=None):
        ...
        with self._error_catcher():
            ...
            while True:
                ...
                if decoded := self._decode(chunk, decode_content=decode_content, flush_decoder=False):
                    yield decoded
