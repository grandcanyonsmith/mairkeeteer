...
def _make_attr_tuple_class(cls_name, attr_names):
    ...
    if attr_names:
        attr_class_template.extend(f"    {attr_name} = _attrs_property(_attrs_itemgetter({i}))" for i, attr_name in enumerate(attr_names))
...
def _transform_attrs(
    ...
    *collect_by_mro* is True, collect them in the correct MRO order, otherwise
    ...
    attrs = [a if a.alias else a.evolve(alias=_default_init_alias_for(a.name)) for a in attrs]
...
class _ClassBuilder:
        ...
        if frozen:
            ...
            for a in attrs:
                ...
                if has_validator and has_converter:
                    break
            if on_setattr == _ng_default_on_setattr and not has_validator and not has_converter or (on_setattr == setters.validate and not has_validator) or (on_setattr == setters.convert and not has_converter):
    ...
    def _create_slots_class(self):
        ...
        for base_cls in self._cls.__mro__[1:-1]:
            ...
            existing_slots |= {name: getattr(base_cls, name) for name in getattr(base_cls, "__slots__", [])}
...
def _make_hash(cls, attrs, frozen, cache_hash):
    ...
    def append_hash_computation_lines(prefix, indent):
        ...
        method_lines.extend([indent + prefix + hash_func, f"{indent}        {type_hash},"])
...
def _make_repr(attrs, ns, cls):
    ...
    globs = {f"{name}_repr": r for name, r, _ in attr_names_with_reprs if r != repr}
    ...
    for a in attrs:
        ...
        if a.init is False:
            if has_factory:
                ...
                if a.converter is not None:
                    lines.append(fmt_setter_with_converter(attr_name, f"{init_factory_name}({maybe_self})", has_on_setattr))

                    conv_name = _init_converter_pat % (a.name,)
                    names_for_globals[conv_name] = a.converter
                else:
                    lines.append(fmt_setter(attr_name, f"{init_factory_name}({maybe_self})", has_on_setattr))
            ...
            if a.converter is not None:
                ...
                lines.append("else:")
                lines.append(
                    f'    {fmt_setter_with_converter(attr_name, f"{init_factory_name}({maybe_self})", has_on_setattr)}'
                )
                ...
                lines.append("else:")
                lines.append(
                    f'    {fmt_setter(attr_name, f"{init_factory_name}({maybe_self})", has_on_setattr)}'
                )
