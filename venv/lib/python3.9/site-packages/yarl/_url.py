...
class URL:
    ...
    def __le__(self, other):
        return NotImplemented if type(other) is not URL else self._val <= other._val
    ...
    def is_default_port(self):
        ...
        return False if default is None else self.port == default
    ...
    def host(self):
        ...
        if raw is None:
            return None
        return raw if "%" in raw else _idna_decode(raw)
    ...
    def raw_path_qs(self):
        ...
        return f"{self.raw_path}?{self.raw_query_string}" if self.raw_query_string else self.raw_path
    ...
    def raw_suffixes(self):
        ...
        if name.endswith("."):
            return ()
        name = name.lstrip(".")
        return tuple(f".{suffix}" for suffix in name.split(".")[1:])
    ...
    def _query_seq_pairs(cls, quoter, pairs):
        for key, val in pairs:
            if isinstance(val, (list, tuple)):
                for v in val:
                    yield f"{quoter(key)}={quoter(cls._query_var(v))}"
            else:
                yield f"{quoter(key)}={quoter(cls._query_var(val))}"
    ...
    def _get_str_query(self, *args, **kwargs):
        if kwargs and args or not kwargs and len(args) != 1:
            raise ValueError(
        ...
        if query is None:
            ...
            # `_query_seq_pairs`.
            query = "&".join(f"{quoter(k)}={quoter(self._query_var(v))}" for k, v in query)
        else:
            raise TypeError(
    ...
    def human_repr(self):
        ...
        query_string = "&".join(f'{_human_quote(k, "#&+;=")}={_human_quote(v, "#&+;=")}' for k, v in self.query.items())
