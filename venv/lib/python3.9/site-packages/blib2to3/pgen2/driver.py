    ...
    def parse_tokens(self, tokens: Iterable[GoodTokenInfo], debug: bool = False) -> NL:
        ...
        for quintuple in proxy:
            ...
            if p.addtoken(cast(int, type), value, (prefix, start)):
                ...
                break
            prefix = _prefix if type in {token.INDENT, token.DEDENT} else ""
            lineno, column = end
            if value.endswith("\n"):
                lineno += 1
                column = 0
...
def _generate_pickle_name(gt: Path, cache_dir: Optional[Path] = None) -> Text:
    ...
    return os.path.join(cache_dir, os.path.basename(name)) if cache_dir else name
...
def _newer(a: Text, b: Text) -> bool:
    ...
    if not os.path.exists(a):
        return False
    return os.path.getmtime(a) >= os.path.getmtime(b) if os.path.exists(b) else True
