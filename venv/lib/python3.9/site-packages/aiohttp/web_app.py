...
class Application(MutableMapping[str, Any]):
    ...
    def __init_subclass__(cls: Type["Application"]) -> None:
        warnings.warn(f"Inheritance class {cls.__name__} from web.Application is discouraged", DeprecationWarning, stacklevel=2)

    if DEBUG:  # pragma: no cover

        def __setattr__(self, name: str, val: Any) -> None:
            if name not in self.ATTRS:
                warnings.warn(f"Setting custom web.Application.{name} attribute is discouraged", DeprecationWarning, stacklevel=2)
    ...
    def _make_handler(
        ...
        **kwargs: Any,
        ...
        if not issubclass(access_log_class, AbstractAccessLogger):
            raise TypeError(f"access_log_class must be subclass of aiohttp.abc.AbstractAccessLogger, got {access_log_class}")
