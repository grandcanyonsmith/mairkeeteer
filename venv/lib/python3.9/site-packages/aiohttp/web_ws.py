...
class WebSocketResponse(StreamResponse):
    ...
    def _handshake(
        ...
        if headers.get(hdrs.UPGRADE, "").lower().strip() != "websocket":
            raise HTTPBadRequest(text=f'No WebSocket UPGRADE hdr: {headers.get(hdrs.UPGRADE)}\n Can "Upgrade" only to "WebSocket".')


        if "upgrade" not in headers.get(hdrs.CONNECTION, "").lower():
            raise HTTPBadRequest(text=f"No CONNECTION upgrade hdr: {headers.get(hdrs.CONNECTION)}")
