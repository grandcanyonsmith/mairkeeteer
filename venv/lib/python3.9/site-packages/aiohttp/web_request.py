...
_QDTEXT: Final[str] = f'[{"".join(chr(c) for c in (9, 32, 33) + tuple(range(35, 127)))}]'
...
class BaseRequest(MutableMapping[str, Any], HeadersMixin):
    ...
    def http_range(self) -> slice:
        ...
        if rng is not None:
            ...
            except IndexError as e:  # pattern was not found in header
    ...
    if DEBUG:
        ...
        def __setattr__(self, name: str, val: Any) -> None:
            if name not in self.ATTRS:
                warnings.warn(f"Setting custom {self.__class__.__name__}.{name} attribute is discouraged", DeprecationWarning, stacklevel=2)
