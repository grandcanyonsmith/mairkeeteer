    ...
    try:
        if host is not None:
            if isinstance(host, (str, bytes, bytearray, memoryview)):
                ...
                sites.extend(TCPSite(runner, h, port, shutdown_timeout=shutdown_timeout, ssl_context=ssl_context, backlog=backlog, reuse_address=reuse_address, reuse_port=reuse_port,) for h in host)
        ...
        if path is not None:
            if isinstance(path, (str, bytes, bytearray, memoryview)):
                ...
                sites.extend(UnixSite(runner, p, shutdown_timeout=shutdown_timeout, ssl_context=ssl_context, backlog=backlog,) for p in path)
        ...
        if sock is not None:
            if not isinstance(sock, Iterable):
                ...
                sites.extend(SockSite(runner, s, shutdown_timeout=shutdown_timeout, ssl_context=ssl_context, backlog=backlog,) for s in sock)
