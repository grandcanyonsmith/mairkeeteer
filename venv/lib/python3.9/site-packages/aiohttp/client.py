...
class ClientSession:
    ...
    def __init_subclass__(cls: Type["ClientSession"]) -> None:
        warnings.warn(f"Inheritance class {cls.__name__} from ClientSession is discouraged", DeprecationWarning, stacklevel=2)

    if DEBUG:

        def __setattr__(self, name: str, val: Any) -> None:
            if name not in self.ATTRS:
                warnings.warn(f"Setting custom ClientSession.{name} attribute is discouraged", DeprecationWarning, stacklevel=2)
    ...
    async def _request(
        ...
        *,
        ...
        if timeout is sentinel:
            ...
            real_timeout = timeout if isinstance(timeout, ClientTimeout) else ClientTimeout(total=timeout)
    ...
    async def _ws_connect(
        ...
        *,
        ...
        try:
            ...
            notakeover = False
            if compress:
                if compress_hdrs := resp.headers.get(hdrs.SEC_WEBSOCKET_EXTENSIONS):
                    try:
