...
def right_hand_split(
    ...
    for leaf in reversed(line.leaves):
        ...
        current_leaves.append(leaf)
        if current_leaves is tail_leaves and leaf.type in CLOSING_BRACKETS and id(leaf) not in omit:
    ...
    *,
    ...
    if component is _BracketSplitComponent.body:
        ...
        if leaves:
            ...
            no_commas = original.is_def and opening_bracket.value == "(" and all(leaf.type != token.COMMA for leaf in leaves) and all(node.prev_sibling.type != RARROW for node in (leaves[0].parent, getattr(leaves[0].parent, "parent", None),) if isinstance(node, Node) and isinstance(node.prev_sibling, Leaf))
...
def remove_await_parens(node: Node) -> None:
    if node.children[0].type != token.AWAIT or len(node.children) <= 1:
        return
    if (
        ...
        if bracket_contents.type != syms.power:
            ensure_visible(opening_bracket)
            ensure_visible(closing_bracket)
        elif bracket_contents.children[0].type == token.AWAIT:
            ensure_visible(opening_bracket)
            ensure_visible(closing_bracket)
    ...
    if node.type == syms.atom:
        ...
        for child in node.children:
            if isinstance(child, Node):
                remove_with_parens(child, node)
    elif node.type == syms.asexpr_test and all(leaf.type != token.COLONEQUAL for leaf in node.leaves()):
        if maybe_make_parens_invisible_in_atom(
            node.children[0],
            parent=node,
            remove_brackets_around_comma=True,
