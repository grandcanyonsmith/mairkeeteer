...
class Line:
    ...
    def opens_block(self) -> bool:
        ...
        return False if len(self.leaves) == 0 else self.leaves[-1].type == token.COLON

    def contains_standalone_comments(self, depth_limit: int = sys.maxsize) -> bool:
        """If so, needs to be split before emitting."""
        return any(leaf.type == STANDALONE_COMMENT and leaf.bracket_depth <= depth_limit for leaf in self.leaves)
    ...
    def contains_uncollapsable_type_comments(self) -> bool:
        ...
        for leaf_id, comments in self.comments.items():
            for comment in comments:
                if is_type_comment(comment) and (comment_seen or (not is_type_comment(comment, " ignore") and leaf_id not in ignored_ids)):
                    return True
    ...
    def has_magic_trailing_comma(
        ...
        return closing.opening_bracket is not None and not is_one_sequence_between(closing.opening_bracket, closing, self.leaves)
...
class EmptyLineTracker:
    ...
    def _maybe_empty_lines(self, current_line: Line) -> Tuple[int, int]:
        ...
        while self.previous_defs and self.previous_defs[-1] >= depth:
            if self.mode.is_pyi:
                ...
                before = 1
            elif self.previous_defs[-1] and current_line.leaves[-1].type == token.COLON and current_line.leaves[0].value not in ("with", "try", "for", "while", "if", "match"):
        ...
        if self.previous_line:
            ...
            if self.previous_line.is_class and current_line.is_triple_quoted_string:
                return before, 1
    ...
    def _maybe_empty_lines_for_class_or_def(
        ...
        if self.previous_line.is_decorator:
            return (0, 1) if self.mode.is_pyi and current_line.is_stub_class else (0, 0)
...
def can_omit_invisible_parens(
    ...
    if first.type in OPENING_BRACKETS and second.type not in CLOSING_BRACKETS and _can_omit_opening_paren(line, first=first, line_length=line_length):
        return True
