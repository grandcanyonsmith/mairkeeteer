...
def read_pyproject_toml(
    ...
    if not config:
        ...
        config = {k: v if isinstance(v, (list, dict)) else str(v) for k, v in config.items()}
...
def validate_cell(src: str, mode: Mode) -> None:
    ...
    if any(transformed_magic in src for transformed_magic in TRANSFORMED_MAGICS):
        raise NothingChanged
    if src.startswith("%%") and src.split()[0][2:] not in PYTHON_CELL_MAGICS | mode.python_cell_magics:
        raise NothingChanged
