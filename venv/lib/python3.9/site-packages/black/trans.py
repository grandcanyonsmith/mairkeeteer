...
def hug_power_op(line: Line, features: Collection[Feature]) -> Iterator[Line]:
    ...
    def is_simple_operand(index: int, kind: Literal["base", "exponent"]) -> bool:
        ...
        if start.type in {token.PLUS, token.MINUS, token.TILDE} and line.leaves[index + 1].type in {token.NAME, token.NUMBER}:
...
class StringMerger(StringTransformer, CustomSplitMapMixin):
    ...
    def _merge_string_group(self, line: Line, string_idx: int) -> TResult[Line]:
        ...
        def make_naked(string: str, string_prefix: str) -> str:
            ...
            naked_string = re.sub(f"({RE_EVEN_BACKSLASHES}){QUOTE}", r"\1\\" + QUOTE, naked_string)
...
class StringParenStripper(StringTransformer):
    ...
    def do_match(self, line: Line) -> TMatchResult:
        ...
        for idx, leaf in enumerate(LL):
            ...
            if (is_valid_index(next_idx) and LL[next_idx].type == token.RPAR and not is_empty_rpar(LL[next_idx])) and (not is_valid_index(next_idx + 1) or LL[next_idx + 1].type not in {token.DOUBLESTAR, token.LSQB, token.LPAR, token.DOT,}):
    ...
    def do_splitter_match(self, line: Line) -> TMatchResult:
        ...
        if string_idx is not None:
            ...
            if " " not in string_value:
                ...
                if len(string_value) > max_string_length and not self.has_custom_splits(string_value):
