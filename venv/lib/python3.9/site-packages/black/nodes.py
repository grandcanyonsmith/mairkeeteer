...
def whitespace(leaf: Leaf, *, complex_subscript: bool) -> str:    # noqa: C901
    ...
    if p.type in {syms.parameters, syms.arglist}:
        ...
        if t == token.EQUAL and prev.type not in TYPED_NAMES or t != token.EQUAL and prev.type != token.EQUAL and prev.type != token.COMMA:
            return NO
...
def is_docstring(leaf: Leaf) -> bool:
    ...
    return bool(prev_siblings_are(leaf.parent, [syms.parameters, token.COLON, syms.simple_stmt]))
...
def is_type_comment(leaf: Leaf, suffix: str = "") -> bool:
    ...
    t = leaf.type
    v = leaf.value
    return t in {token.COMMENT, STANDALONE_COMMENT} and v.startswith(f"# type:{suffix}")
